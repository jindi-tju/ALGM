...
class FastCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for module in self.fast_cnn:
            x_out.append(module(x).squeeze().cuda())
...
class MultiDetModule(nn.Module):
    def __init__(self, feature_dim=64+16+16, h_dim=64):
        super(MultiDetModule, self).__init__()
        self.encoding = EncodingPart()
    ...
    def forward(self, text_raw, image_raw):
        ...
        text, image = self.similarity_module(text_raw, image_raw)
        correlation = self.cross_module(text, image)

        a = self.ambiguity_module(text, image)
        weight_uni = (1-a).unsqueeze(1)
        weight_corre = a.unsqueeze(1)
